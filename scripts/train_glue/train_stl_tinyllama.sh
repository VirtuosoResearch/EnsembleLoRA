task_names=("winogrande_debiased" "story_cloze"  "multirc") # "cb" "rte" "copa" "wic" "wsc.fixed"
length=${#task_names[@]}
...
for ((j = 0; j < $length; j++)); do
  python custom_train_glue.py --task_name "${task_names[$j]}" \
      ...
      --devices 2 3 --batch_size 8 --inference_batch_size 8 --max_length 512 --runs 2 --lr 5e-5\
      --train_lora --lora_rank 16 --lora_alpha 128 \
      --save_name pairwise --epochs 10 --write_results
