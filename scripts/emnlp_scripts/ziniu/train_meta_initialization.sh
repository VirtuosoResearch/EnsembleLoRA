python custom_train_alpaca.py --model_key "EleutherAI/gpt-neo-2.7B" \
    --lr 5e-5 --batch_size 16 --max_length 256 --epochs 10\
    --train_lora --lora_rank 4 --lora_alpha 32\
    --strategy auto --devices 0  --runs 1 --precision "bf16-true" --accumulate 1 --save_every_epoch --save_name meta_initialization_alpaca

# python custom_train_alpaca.py --model_key "EleutherAI/gpt-neo-1.3B"\
#     --lr 5e-5 --batch_size 16 --max_length 256 --epochs 10\
#     --train_lora --lora_rank 4 --lora_alpha 32\
#     --strategy auto --devices 0 --runs 1 --precision "bf16-true" --accumulate 1 --save_every_epoch --save_name quantized_meta_initialization\
#     --use_qlora --optimizer "paged_adamw_8bit"

# python custom_train_alpaca.py --model_key "EleutherAI/gpt-neo-125m"\
#     --lr 5e-5 --batch_size 16 --max_length 256 --epochs 10\
#     --train_lora --lora_rank 4 --lora_alpha 32\
#     --strategy auto --devices 0 --runs 1 --precision "bf16-true" --accumulate 1 --save_every_epoch --save_name quantized_meta_initialization\
#     --use_qlora --optimizer "paged_adamw_8bit"

# python custom_train_alpaca.py --model_key "TinyLlama/TinyLlama-1.1B-intermediate-step-1431k-3T"\
#     --lr 5e-5 --batch_size 16 --max_length 256 --epochs 10\
#     --train_lora --lora_rank 4 --lora_alpha 32\
#     --strategy auto --devices 0 --runs 1 --precision "bf16-true" --accumulate 1 --save_every_epoch --save_name quantized_meta_initialization\
#     --use_qlora --optimizer "paged_adamw_8bit"